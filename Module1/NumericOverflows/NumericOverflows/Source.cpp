...
// 3/7/2024 Module 1 CS405-Secure Coding
...
{
    // return a tuple with result and a boolean status indicator for overflow
    T result = start;
    bool overSuccess = true; // true when operation performed successfully without overflow

    for (unsigned long int i = 0; i < steps; ++i)
    {
        // check for possible overflow by ensuring not to go over numeric_limit max
        if (std::numeric_limits<T>::max() - increment < result) {
            overSuccess = false;
            break; // overflow is detected, break
        }
        result += increment;
    }

    return { result, overSuccess }; // return result AND status
}
...
{
    // return a tuple with result and a boolean status indicator for underflow
    T result = start;
    bool underSuccess = true; // boolean to detect if operation without underflow was successful

    for (unsigned long int i = 0; i < steps; ++i)
    {
        // check for possible underflow
        if (decrement > result || result - decrement < std::numeric_limits<T>::lowest())
        {
            underSuccess = false; // switch the flag and return
            break;
        }
        result -= decrement;
    }

    return { result, underSuccess };
}
...
void test_overflow()
{
    ...
    if (result.second) { // if second argument (success) is true, no overflow
        std::cout << +result.first << std::endl;
    }
    else {
        std::cout << "Overflow detected" << std::endl; // inform user in case (shouldn't occur here)
    }
    ...
    if (result.second) {
        ...
        std::cout << "Overflow detected" << std::endl; // inform user
    }
}
...
void test_underflow()
{
    ...
    if (result.second) { // check boolean
        std::cout << +result.first << std::endl;
    }
    else {
        std::cout << "Underflow detected" << std::endl; // indicate an underflow occurred (shouldn't occur here)
    }
    ...
    if (result.second) {
        ...
        std::cout << "Underflow detected" << std::endl; // inform user of underflow
    }
}

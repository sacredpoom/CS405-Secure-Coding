...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // This assignment only allows modifications in the run_query method 
    // Ideally using prepared statements would take place in the run_queries method
    // Any new types of injections will not be prevented by implemented fix
    // unless also updated in this function

    // search for the base query
    if (sql.find("SELECT ID, NAME, PASSWORD FROM USERS WHERE NAME=") == std::string::npos)
    {
        ...
        std::string sqlPrepare = "SELECT ID, NAME, PASSWORD FROM USERS WHERE NAME = ?"; // Prepare a statement to use
        std::string sqlEdited = sql.substr(48, sql.length());                           // Skip first 48 char (up to the =) pull remaining
        std::string sqlStrip;                                                           // for after removing apostrophes
        std::string sqlFinal;                                                           // for the final prepared statement

        // remove the ' apostrophe
        for (size_t i = 0; i < sqlEdited.size(); ++i) {
            if ((sqlEdited[i] != '\'')) {
                sqlStrip += sqlEdited[i];
            }
        }

        std::cout << std::endl;
        std::cout << "Attempted Query: " << sqlEdited; // for testing purposes show what being attempted

        // heuristic search for common injection patterns
        // just additional layer of security on top of using prepared statement
        // use of prepared statement prevents input from running as part of SQL code 
        // instead being treated as a value. This just helps for detection.
        if ((sqlStrip.find(" or ") != std::string::npos ||
             ...
             sqlStrip.find(';') != std::string::npos))
        {
            // inform user of possible attack
            std::cout << std::endl;
            std::cout << "Possible SQL Injection attempt detected!";
        }

        // statement pointer
        sqlite3_stmt* stmt;
        // parse statement into string
        sqlite3_prepare_v2(
            db,
            sqlPrepare.c_str(),
            sqlPrepare.length(),
        ...
        // bind search into prepared statement
        sqlite3_bind_text(
            stmt,
            1,
            sqlStrip.c_str(),
        ...
        // store final prepared SQL query
